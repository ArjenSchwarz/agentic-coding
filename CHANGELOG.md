...
### Changed
- `pr-pilot` skill: Detect `anthropics/claude-code-action` references in `.github/workflows/*.yml`, `*.yaml`, `*.yml.disabled`, and `*.yaml.disabled` files (new step 1.5) and, when present, invoke the `local-review` agent in steps 2.1 and 3.3 so the Claude review comment is posted locally instead of via the GitHub Action. The marker file's *presence* is the opt-in signal — disabling the upstream Action by renaming the workflow to `*.yml.disabled` is the recommended setup for local-only Claude reviews and keeps `local-review` active. Filename filter still excludes `.bak`, editor swap files, and other unintentional matches
