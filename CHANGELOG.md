...
### Changed
- `pr-pilot` skill: Detect `anthropics/claude-code-action` references in active `.github/workflows/*.yml`/`*.yaml` files (new step 1.5) and, when present, invoke the `local-review` agent in steps 2.1 and 3.3 so the Claude review comment is posted locally instead of via the GitHub Action. Skipped when no such workflow is present. Filename filter excludes inactive files (`*.yml.disabled`, `*.bak`, editor swap files) that GitHub Actions does not execute. Note explains that suppressing the upstream workflow trigger is a separate step the user must take for the savings to be real — and that renaming the workflow to `*.yml.disabled` also turns off local-review, so for local-only Claude reviews keep the file as `*.yml` and gate its `on:` trigger instead
