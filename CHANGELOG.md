...
### Changed
- `pr-pilot` skill: Detect `anthropics/claude-code-action` in `.github/workflows/` (new step 1.5) and, when present, invoke the `local-review` agent in steps 2.1 and 3.3 so the Claude review comment is posted locally instead of via the GitHub Action. Skipped when no such workflow is present. Note explains that suppressing the upstream workflow trigger is a separate step the user must take for the savings to be real
